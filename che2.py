...
Alias: alias che='source ~/compartido/amor/config/che.conf && python3 ~/compartido/amor/che.py'
...
PERFILES: cada alias (che, amor, etc.) puede cargar un .conf distinto
con su propio modelo, temperatura, tono y si busca en internet o no.
Ver ~/compartido/amor/config/*.conf

BÚSQUEDA WEB: si SEARCH_PROVIDER está seteado (tavily o brave) y la
pregunta tiene palabras como "hoy", "último", "precio", "noticias",
busca en internet antes de responder y le pasa el resultado al modelo.

KEYS necesarias en ~/.bashrc:
  export GROQ_API_KEY="gsk_..."
  export GEMINI_API_KEY="AI..."
  export TAVILY_API_KEY="tvly-..."   (opcional, para búsqueda web)
  export BRAVE_API_KEY="BSA..."      (opcional, alternativa a Tavily)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
import acciones_archivos as archivos  # módulo aparte: lectura/escritura de archivos
...
# Perfil del asistente — lo define el .conf que cargó el alias (che/amor/etc)
# Si no se cargó ningún .conf, usa estos valores por defecto (igual que antes)
CHE_PERFIL       = os.environ.get("CHE_PERFIL", "Che")
CHE_PERSONALIDAD = os.environ.get("CHE_PERSONALIDAD", "")
CHE_MODELO_GROQ  = os.environ.get("CHE_MODELO_GROQ", "llama-3.3-70b-versatile")
CHE_TEMPERATURA  = float(os.environ.get("CHE_TEMPERATURA", "0.1"))
CHE_MAX_TOKENS   = int(os.environ.get("CHE_MAX_TOKENS", "400"))
CHE_BUSQUEDA     = os.environ.get("CHE_BUSQUEDA", "true").lower() == "true"

# Búsqueda web en tiempo real — Tavily o Brave (ambas con free tier)
SEARCH_PROVIDER      = os.environ.get("SEARCH_PROVIDER", "none").lower()  # tavily | brave | none
TAVILY_API_KEY       = os.environ.get("TAVILY_API_KEY", "")
BRAVE_API_KEY        = os.environ.get("BRAVE_API_KEY", "")
SEARCH_MAX_RESULTS   = int(os.environ.get("SEARCH_MAX_RESULTS", "3"))
SEARCH_CONTEXT_CHARS = int(os.environ.get("SEARCH_CONTEXT_CHARS", "2000"))
...
# ══════════════════════════════════════════════════════════════
# SECCIÓN 6B: BÚSQUEDA WEB EN TIEMPO REAL
# Si la pregunta tiene palabras de "info fresca", busca en internet
# antes de armar el prompt y le pasa el resultado al modelo
# ══════════════════════════════════════════════════════════════

PALABRAS_BUSQUEDA = [
    "hoy", "ahora", "actual", "actuales", "último", "ultima", "última", "últimos",
    "recién", "recien", "precio", "cotización", "cotizacion", "dólar", "dolar",
    "noticia", "noticias", "qué pasó", "que paso", "esta semana", "este mes",
    "este año", "en vivo", "resultado", "resultados",
]

def necesita_busqueda(pregunta):
    """Decide si la pregunta necesita info fresca de internet."""
    if not CHE_BUSQUEDA or SEARCH_PROVIDER == "none":
        return False
    p = pregunta.lower()
    return any(palabra in p for palabra in PALABRAS_BUSQUEDA)

def buscar_tavily(query):
    """Busca en internet con Tavily — devuelve resultados ya armados para IA."""
    ...
    try:
        ...
        for res in data.get("results", []):
            partes.append(f"- {res.get('title')}: {res.get('content', '')[:300]} ({res.get('url')})")
        return "\n".join(partes)[:SEARCH_CONTEXT_CHARS]
    except Exception as e:
        print(f"⚠️ Tavily falló: {e}")
        return ""

def buscar_brave(query):
    """Busca en internet con la API de Brave Search."""
    ...
    try:
        ...
        for res in data.get("web", {}).get("results", []):
            partes.append(f"- {res.get('title')}: {res.get('description', '')} ({res.get('url')})")
        return "\n".join(partes)[:SEARCH_CONTEXT_CHARS]
    except Exception as e:
        print(f"⚠️ Brave falló: {e}")
        return ""

def buscar_web(query):
    """Despacha al proveedor configurado en SEARCH_PROVIDER."""
    if SEARCH_PROVIDER == "tavily":
        return buscar_tavily(query)
    elif SEARCH_PROVIDER == "brave":
        return buscar_brave(query)
    return ""
...
def construir_prompt_sistema(contextos, pregunta):
    """
    ...
    """
    base = (
        f"Sos '{CHE_PERFIL}', un asistente de terminal experto en Linux, redes y Python. "
        ...
        "Si te piden generar o modificar un archivo, proponé el contenido completo en un bloque "
        "```escribir:/ruta/al/archivo\\ncontenido\\n``` y el sistema le va a preguntar al usuario si lo guarda. "
        "Si en el contexto de abajo aparece el contenido de algún archivo (bloques '--- ruta ---'), "
        "ya lo tenés disponible para leer y analizar, no hace falta pedir que te lo peguen. "
        "Si necesitás más contexto para responder bien, hacé UNA sola pregunta concreta."
    )
    if CHE_PERSONALIDAD:
        base += " " + CHE_PERSONALIDAD
    ...
    # Archivos mencionados en la pregunta (ej: "leé che.py", "revisá ~/compartido/amor/bot_che.py")
    contenido_archivos = archivos.contexto_de_archivos(pregunta)
    if contenido_archivos:
        extras.append(f"=== ARCHIVOS MENCIONADOS ===\n{contenido_archivos}")

    # Búsqueda web solo si la pregunta pide info fresca y hay proveedor configurado
    if necesita_busqueda(pregunta):
        print("🌐 Buscando en internet...")
        resultado_web = buscar_web(pregunta)
        if resultado_web:
            extras.append(
                "=== BÚSQUEDA WEB (info actual de internet, aclarale al usuario que es de internet) ===\n"
                f"{resultado_web}"
            )
...
def llamar_groq(pregunta, sys_prompt):
    ...
    try:
        r = requests.post(
            ...
            json={
                "model": CHE_MODELO_GROQ,
                "messages": [
                    {"role": "system", "content": sys_prompt},
                    {"role": "user",   "content": pregunta}
                ],
                "temperature": CHE_TEMPERATURA,
                "max_tokens": CHE_MAX_TOKENS
            },
            timeout=12
        )
        if r.status_code == 200:
            return r.json()['choices'][0]['message']['content'].strip(), f"Groq/{CHE_MODELO_GROQ} ⚡"
...
def main():
    if len(sys.argv) < 2:
        print(f"{CHE_PERFIL}, faltó la pregunta. Ej: che como veo mis procesos")
        sys.exit(1)
    ...
    # ── Paso 4: armar el system prompt con contexto relevante (y búsqueda web si aplica) ──
    sys_prompt = construir_prompt_sistema(contextos, pregunta)
    ...
    if respuesta:
        ...
        archivos.ofrecer_escritura(respuesta)
    else:
